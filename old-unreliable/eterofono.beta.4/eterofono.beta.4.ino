...
const float PHASE_SHIFT_MAX = 15.0;
...
/*
 * Sensor calibration setup routine 
 */
void SensorCalibrationLoop(){
  
  digitalWrite(LED_PORT, HIGH);

  while (millis() < SETUP_LOOP_DURATION_MILLIS){

    int sensorValue = sense();
  ...
  port_threshold = int(port_range_min + 0.01*(port_range_max - port_range_min));
  
  digitalWrite(LED_PORT, LOW);
  Serial.println("SETUP: ");
  Serial.println(port_range_min);
  Serial.println(port_range_max);
  Serial.println(port_threshold);
}
 ...
 ISR(TIMER1_OVF_vect) {   
  ...
  Serial.println(OCR1A); // VALUE DEBUG ONLY!
 }  
...
int sense(){
  ...
  int i;
  for (i=0; i< NMIS; i++){
    sensor = sensor +  (float) analogRead(SENSOR_PORT);
    delay(1);
  }
  sensor = int(sensor/NMIS);
  */

  /*
  long sensor =  cs42.capacitiveSensor(255);
...
void loop() {
  ...
  int i;
  for (i=0; i< NMIS; i++){
    sensor = sensor +  analogRead(SENSOR_PORT);
    delay(5);
  }
